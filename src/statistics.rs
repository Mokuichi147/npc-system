...
impl YearStatistics {
    pub fn new(year: u16, total_population: usize, town_populations: Vec<usize>) -> Self {
        Self {
            year,
            total_population,
            town_populations,
...
impl Statistics {
    ...
    pub fn cumulative(&self) -> CumulativeStatistics {
        let mut result = CumulativeStatistics {
            initial_population: self.initial_population,
            final_population: self
                .latest()
                .map_or(self.initial_population, |year| year.total_population),
            total_unique_npcs: self.total_unique_npcs,
    ...
    pub fn detect_warnings(&self, health: SimulationHealthMetrics) -> Vec<SimulationWarning> {
        ...
        if !self.years.is_empty() && cumulative.goal_changes == 0 {
            warnings.push(SimulationWarning::NoGoalChanges);
        }

        let simulated_years = self.years.len();
